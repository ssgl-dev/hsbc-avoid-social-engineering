const TIMELINE_URL = '/static/timeline-credit-card.json';
const VIDEO_URL = '/static/videos/credit-card-total.mp4';
...
function setActiveItem(index, scroll = false) {
  ...
  revealAncestorsForElement(items[activeIndex].el);
...
function revealAncestorsForElement(el) {
  if (!(el instanceof Element)) return;

  const expContent = el.closest('.exp-content');
  if (expContent) {
    expContent.classList.add('expanded');
    expContent.setAttribute('aria-hidden', 'false');
    const dropdown = expContent.closest('.expander')?.querySelector('.dropdown');
    if (dropdown) {
      dropdown.setAttribute('aria-expanded', 'true');
      const label = dropdown.querySelector('.dropdown-text');
      if (label) label.classList.add('expanded');
    }
  }

  const tabContent = el.closest('[role="tabpanel"]');
  if (tabContent && tabContent.id) {
    const panelId = tabContent.id;
    const tabLink = document.querySelector('.tab-widget-link[data-target="#' + CSS.escape(panelId) + '"]');
    if (tabLink && tabLink.getAttribute('aria-selected') !== 'true') {
...
function mapNodesToPage(nodes) {
  items = nodes.map((node, index) => {
    ...
    const selector = isEn && node.selector_en ? node.selector_en : node.selector;
